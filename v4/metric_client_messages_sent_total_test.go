...
func TestNewClientMessagesSentTotalStatsHandler(t *testing.T) {
	...
	h := promgrpc.NewStatsHandler(
		promgrpc.NewClientMessagesSentTotalStatsHandler(
			promgrpc.NewClientMessagesSentTotalCounterVec(collectorOpts...),
			statsHandlerOpts...,
		))
	...
	expected := fmt.Sprintf(`
		grpc_client_messages_sent_total{%s="%s",grpc_client_user_agent="fake-user-agent",grpc_is_fail_fast="true",grpc_method="Method",grpc_service="service"} 3
